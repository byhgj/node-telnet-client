...
module.exports = class Telnet extends events.EventEmitter {
  ...
  connect(opts) {
    return new Promise((resolve, reject) => {
	  ...
	  this.enablePrompt = (typeof opts.enablePrompt !== 'undefined' ? opts.enablePrompt : />$/);
	  this.enable = (typeof opts.enable != null ? opts.enable : null);
	  this.enablePasswordPrompt = (typeof opts.enablePasswordPrompt !== 'undefined' ? opts.enablePasswordPrompt : /Password[: ]*$/i);
	  this.enablePassword = (typeof opts.enablePassword != null ? opts.enablePassword : null);
      ...
      this.socket.on('data', data => {
		  if (this.debug) {
			  console.log('STATE='.cyan.bold,this.state);
			  process.stdout.write(data.toString());
		  }
        if (this.state === 'standby') {
			if (utils.search(data.toString(), this.pageSeparator) != -1){
				this.socket.write(Buffer.from('20', 'hex'));
				this.state = 'response';
			}
			//if (utils.search(data.toString(), this.shellPrompt) != -1){
			//	this.exec('');
			//}
  ...
  exec(cmd, opts, callback) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket.write(cmd, () => {
		...
		if (this.debug) {
			console.log('WRITE TO SOCKET:'.green.bold, cmd);
			console.log('add once/responseready'.cyan.bold);
		}
        ...
        if (this.execTimeout) {
          execTimeout = setTimeout(() => {
			...
			if (this.debug) {
				console.log(('* '.repeat(10) + 'remove/responseready' + ' *'.repeat(10)).cyan.bold);
			}
            ...
            //if (!this.response) return reject(new Error('response not received 2'))
          }, this.execTimeout)
        }
        ...
        function responseHandler() {
			...
			if (this.debug) {
				console.log('once/responseready: run responseHandler'.cyan.bold);
			}
		  if (execTimeout !== null) {
        ...
        function buffExcHandler() {
          ...
          if (!this.inputBuffer) return reject(new Error('response not received 3'))
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.socket.writable) {
        this.socket.write(data, () => {
		  ...
		  if (this.debug) {
			  console.log('add on/data to sendHandler'.cyan.bold);
		  }
          ...
          if ((opts && opts.waitfor === undefined) || !opts) {
            setTimeout(() => {
				if (this.debug) {
					console.log('remove on/data to sendHandler'.cyan.bold);
				}
				
              if (response === '') {
                  this.removeListener('data', sendHandler)
                  reject(new Error('response not received 1'))
                  return
              }
          ...
          function sendHandler(data) {
			  if (this.debug) {
				  console.log('run sendHandler'.cyan.bold);
			  }
  ...
  _parseData(chunk, callback) {
    ...
    if (this.state === 'getprompt') {
	  ...
	  // add enable function, by byhgj
	  else if (this.enable != null && utils.search(stringData, this.enablePrompt) !== -1){
		  this.state = 'login'
		  this._login('enable')
	  }
	...
	else if (this.state === 'getenprompt') {
		const stringData = chunk.toString()
		if (utils.search(stringData, this.enablePasswordPrompt) !== -1) {
			this.state = 'login'
			this._login('enablePassword')
		}
	}
	  ...
	  if (this.debug) {
		console.log('state/response -> event/responseready'.cyan.bold, this.listeners('responseready'));
	  }
      this.emit('responseready')
    }
  }
  ...
  _login(handle) {
    if ((handle === 'username' || handle === 'password' || handle === 'enable' || handle === 'enablePassword') && this.socket.writable) {
      this.socket.write(this[handle] + this.ors, () => {
		  // by byhgj
        if (this.handle === 'enable' && this.enablePassword != null) this.state = 'getenprompt';
		else this.state = 'getprompt';
      })
    }
  }
